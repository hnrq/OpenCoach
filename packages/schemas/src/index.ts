...
export const MeasuresSchema = z.object({
	...
	mandatory_sites: z.object({
		...
		waist_narrowest: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
		umbilical: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
		hip_widest: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
		thigh_mid: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
		bicep_flexed: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
		forearm: z.number().positive({ message: "must be > 0" }).describe("Skinfold measurement in mm"),
	}).describe("7-site skinfold protocol measurements, all in millimetres"),
