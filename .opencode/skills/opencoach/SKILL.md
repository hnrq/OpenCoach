...
### Planning
Diet and training plans are generated by the Head Coach agent during appointments.
The `generate-diet` and `generate-training` scripts have been removed — they used a
legacy flat schema and random exercise selection incompatible with the current methodology.
